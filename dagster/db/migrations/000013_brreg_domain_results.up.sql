CREATE TABLE IF NOT EXISTS dagster_brreg.domain_results (
  ...
  CONSTRAINT chk_dagster_brreg_domain_results_status CHECK (
    status IN ('succeeded', 'not_found', 'partial', 'failed')
  ),
  CONSTRAINT chk_dagster_brreg_domain_results_payload_object CHECK (jsonb_typeof(domain_payload) = 'object'),
  CONSTRAINT chk_dagster_brreg_domain_results_metadata_object CHECK (jsonb_typeof(metadata) = 'object')
);

CREATE INDEX IF NOT EXISTS idx_dagster_brreg_domain_results_raw_created
  ON dagster_brreg.domain_results (raw_record_id, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_dagster_brreg_domain_results_status
  ON dagster_brreg.domain_results (status, created_at DESC);
